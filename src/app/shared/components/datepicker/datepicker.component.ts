...
import { toBoolean } from '../utilities/convert';
import { pgTimePickerInnerComponent } from '../time-picker/timepicker-inner.component';
import { DEFAULT_DATEPICKER_POSITIONS } from '../utilities/overlay-position-map';
...
export class pgDatePickerComponent implements ControlValueAccessor, OnInit {
  ...
  @ViewChild(pgTimePickerInnerComponent, {static: true}) timePickerInner: pgTimePickerInnerComponent;
  @ViewChild('monthSlider', {static: true}) _monthSlider: ElementRef;
  @Input()
  ...
  set FormControl(value: FormControl) {
    ...
    this._formControl.valueChanges.pipe(map(value => this._value = value)).subscribe();
  }
  ...
  private _valueBehaviorSubject : BehaviorSubject<Date> = new BehaviorSubject(new Date);
  get Value(): Date {
  ...
  public get Value$() : Observable<Date> {
    return this._valueBehaviorSubject.asObservable();
  }
  ...
  private _updateValue(value: Date): void {
    ...
    this._valueBehaviorSubject.next(this._value);
